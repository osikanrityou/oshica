...
export default async function GoodsPage() {
  ...
  return (
    <main className="mx-auto max-w-md px-5 py-8">
      ...
      <p className="mt-2 text-sm text-zinc-500">登録したグッズを管理できます</p>
      ...
      <div className="mt-6 space-y-3">
        {goods && goods.length > 0 ? (
          ...
          <p className="text-sm text-zinc-500">グッズを登録しましょう</p>
        )}
      </div>

      <Link
        href="/goods/new"
        className="fixed bottom-28 right-6 flex h-14 w-14 items-center justify-center rounded-full bg-sky-400 text-3xl text-white shadow-lg"
      >
        +
      </Link>
    </main>
